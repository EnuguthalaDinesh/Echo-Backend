...
async def get_user_chat_history(
    ...
    """
    Retrieves the entire persistent chat history for the logged-in user 
    by fetching all messages from their associated tickets.
    """
    if not current_user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not authenticated")
    
    # Get the most reliable customer identifier (MongoDB ObjectId or external 'id' field)
    customer_id = current_user.get("id") or str(current_user.get("_id"))
    ...
    try:
        ...
        # Note: We query by the most reliable ID: the one returned from /me and /login
        cursor = tickets_col.find({"customer_id": customer_id}).sort("created_at", -1)
        ...
        async for ticket in cursor:
            ...
            # Extract conversation history from each ticket
            for message in ticket.get("conversation_history", []):
                # Ensure the message has a role and content
                if message.get("role") and message.get("content"):
                    history_list.append(HistoryMessage(
                        session_id=ticket_id, # Use ticket ID as a session grouping
                        role=message['role'],
                        content=message['content'],
                        # Ensure timestamp is a datetime object for Pydantic to handle
                        timestamp=message.get('timestamp', datetime.utcnow()), 
                        meta={"ticket_id": ticket_id, "subject": ticket_subject}
                    ))
        
        # Sort all messages by timestamp regardless of the ticket they came from
        history_list.sort(key=lambda x: x.timestamp, reverse=True)

        return history_list
        
    except Exception as e:
        logging.error(f"Error fetching user chat history from tickets: {e}")
